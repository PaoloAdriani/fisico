...
if (orderHeader) {
    ...
    //@mpstyle Order email
    orderEmail = orderReadHelper.getOrderEmail()
    ...
    //@mpstyle Customer phone number
    GenericValue partyContactMechPurposeFirst = from("PartyContactMechPurpose").where("partyId", partyId, "contactMechPurposeTypeId", "PRIMARY_PHONE").cache(true).filterByDate().queryFirst()
    if(partyContactMechPurposeFirst) {
        String _contactMechId = (String) partyContactMechPurposeFirst.contactMechId
        GenericValue telecomNumber = from("TelecomNumber").where("contactMechId", _contactMechId).cache(true).queryOne()
        String infoString = telecomNumber.contactNumber
