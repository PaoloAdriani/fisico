        ...
        <fo:table table-layout="fixed" border-spacing="3pt">
            ...
            <fo:table-body>
                <#list orderItemList as orderItem>
                    ...
                    <#-- @mpstyle -->
                    <#assign colorIndex =  orderItem.productId.indexOf(".") />
                    <#assign productName = productId.substring(0, colorIndex - 5) />
                    <#assign productColor = productId.substring(colorIndex - 5, colorIndex) />
                    <#assign size = productId?keep_after(".") />
                    ...
                    <#-- @mpstyle Color description -->
                    <#assign productFeatureAndApplFirst = EntityQuery.use(delegator).from("ProductFeatureAndAppl").where("productId", productId, "productFeatureTypeId", "COLOR").queryFirst() />
                    <#assign featureDescription = ""/>
                    <#if productFeatureAndApplFirst?? && productFeatureAndApplFirst?has_content>
                        <#assign featureDescription = productFeatureAndApplFirst.description?default(productFeatureAndApplFirst.productFeatureId) />
                    </#if>
                    ...
                    <fo:table-row>
                        <fo:table-cell>
                            <fo:block>
                                ...
                                <#elseif productId??>
                                    ${orderItem.productId?default("N/A")} / ${productColor!} - ${featureDescription!} / ${size!} - ${orderItem.itemDescription!}
                            ...
                            <#-- @mpstyle Facility reservation data -->
                            ...
                            <#if itemIssuances?? && itemIssuances?has_content>
                                <#list itemIssuances as itemIssuance>
                                    <#if itemIssuance.inventoryItemId?has_content>
                                        ...
                                        <#if !facilityName??>
                                            <#assign facilityName = facility.description?default(facility.facilityId) />
                                        </#if>
                                        <fo:block>
                                            ${uiLabelMap.Facility}: ${facilityName!}
                                        </fo:block>
                                    </#if>
                                </#list>
                            <#else>

                                <#assign orderItemShipGrpInvResList = Static["org.apache.ofbiz.order.order.OrderReadHelper"].getOrderItemShipGrpInvResList(orderItem)>

                                <#if orderItemShipGrpInvResList?has_content>
